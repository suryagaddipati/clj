...
(defn updateCount [m v in]
  (let [prvCnt (get m v 0)
        ...
        addNewCountM (assoc removePrevCountM newCntKey (conj (get m prvCntKey #{} )v))]
   addNewCountM)) 

(defn updateCounti [m v in]
(if (and (not in) (not (contains? m v))) m (updateCount m v in)))
